...
Before your first real review (and whenever you detect the flow-check tooling is missing), get set up by invoking the **`plumber-setup` skill** (`.claude/skills/plumber-setup/SKILL.md`):

1. Check for the tooling: `.dependency-cruiser.cjs`, a `check:flow` script in `package.json`, and a `.claude/topology.md` baseline.
2. If any are missing, invoke the `plumber-setup` skill. It installs dependency-cruiser (+ madge), authors a repo-tuned layering ruleset, wires `check:flow` / `check:cycles`, validates they run, and writes the topology baseline.
...
- run `bun run check:flow` (and `check:cycles`) for graph evidence, and
- read `.claude/topology.md` so you don't re-discover the repo each run.

If the tooling is absent and `plumber-setup` has not been run, you MUST return **UNKNOWN** for any graph-level claim and recommend running `plumber-setup` — never PASS on unverified topology.
...
If the repo lacks automated topology checks, recommend adding one. In THIS repo, that provisioning is exactly what the **`plumber-setup` skill** does (dependency-cruiser + madge + `check:flow`/`check:cycles` + topology baseline) — invoke it instead of hand-rolling a check.
