 ...
 *  3. Emit a startup probe (`logger.info("[pensieve] log capture started")`)
...
const GLOBAL_KEY = Symbol.for("detour.pensieve.logSinks");
...
export class PensieveLogService {
	...
	start(): void {
		...
		try {
			logger.info("[pensieve] log capture started");
		} catch {
