...
export function TrajectoriesPane({ client }: { client: WebClient }) {
	...
	const fetcher = useCallback(
		() =>
			client.activityTrajectories({
				limit: 100,
				...(status ? { status } : {}),
			}),
		[client, status],
	);
	const { data, error } = usePoller<ActivityTrajectoryListResult>(
		fetcher,
		5000,
		[status],
	);
	...
	const handleExportAll = useCallback(async () => {
		setExporting(true);
		try {
			const exported = await client.activityExportTrajectoriesZip();
			...
			<div className="pensieve-split-list">
				<div className="pensieve-toolbar">
					...
					<button
						...
						title="Download every trajectory as a zip"
					>
				...
				<div className="pensieve-trajectories-list">
					{(data?.trajectories ?? []).map((t) => (
						<button
							...
							className={
								selected === t.id
									? "pensieve-trajectory-row active"
									: "pensieve-trajectory-row"
							}
						...
						>
							...
							<div className="hint" style={{ marginTop: 4 }}>
								{t.llmCallCount ?? 0} LLM calls · {t.totalPromptTokens ?? 0}↑ /{" "}
								{t.totalCompletionTokens ?? 0}↓ tokens
							</div>
					...
					{data && data.trajectories.length === 0 && (
						<div className="empty">
							No trajectories yet — send a chat message to generate one.
						</div>
					)}
				</div>
			</div>
			<div className="pensieve-split-detail">
				{selected ? (
					...
					<div className="empty" style={{ marginTop: 40 }}>
						Select a trajectory to inspect its pipeline, LLM calls, and provider
						accesses.
					</div>
				)}
			</div>
