...
- Local agent session capture is managed by XHawk. Keep `.xhawk/`, `.agents/`, `.codex/`, `.cursor/`, and local Claude settings out of commits unless a file is intentionally allowlisted.
- `xh skill install` should report installed for Claude Code, Codex, Cursor, and OpenCode. Re-run it after adding a new coding agent on this machine.
...
- Agent setup is checked with `bun run verify:agents`; run `bun run build:app` after packaging or desktop-shell changes.
