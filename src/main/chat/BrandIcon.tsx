/**
 * BrandIcon — official channel brand SVGs.
 *
 * Sources: simpleicons.org (CC0 — public domain). Fetched via
 *   curl -L https://cdn.simpleicons.org/<slug>
 * and inlined here so the icons ship with the bundle (no CDN dependency
 * at runtime, no flash-of-missing-icon on cold start, no extra network
 * call per render).
 *
 * Each icon is a single `<path d="…">` from the upstream SVG, rendered
 * at 14×14 inside a 24-coord viewBox. `fill` uses the brand color by
 * default and falls back to `currentColor` when `monochrome` is true —
 * matches Detour's existing SidebarIcon pattern so active/hover states
 * Just Work.
 */
...
export function BrandIcon({
	...
	const fill = monochrome ? "currentColor" : BRAND_COLOR[name];
	return (
		<svg
			width={size}
			height={size}
			...
			fill={fill}
			role="img"
			aria-label={BRAND_TITLE[name]}
		>
			<title>{BRAND_TITLE[name]}</title>
			<path d={BRAND_PATH[name]} />
		</svg>
	);
}

export const BRAND_ICON_NAMES: ReadonlyArray<BrandIconName> = [
	"discord",
	"telegram",
	"github",
	"imessage",
