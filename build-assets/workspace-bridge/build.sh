...
SHARED="$HERE/../shared"
APP="$HERE/DetourWorkspace.app"
BIN="$APP/Contents/MacOS/DetourWorkspace"

if ! command -v swiftc >/dev/null 2>&1; then echo "swiftc not found" >&2; exit 1; fi

echo "[workspace] building $APP"
rm -rf "$APP"
...
if command -v codesign >/dev/null 2>&1; then
  codesign --force --sign - --deep "$APP" 2>/dev/null || echo "[workspace] codesign skipped"
fi
echo "[workspace] built $APP"
