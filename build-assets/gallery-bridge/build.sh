...
SHARED="$HERE/../shared"
APP="$HERE/DetourGallery.app"
BIN="$APP/Contents/MacOS/DetourGallery"

if ! command -v swiftc >/dev/null 2>&1; then echo "swiftc not found" >&2; exit 1; fi

echo "[gallery] building $APP"
rm -rf "$APP"
...
if command -v codesign >/dev/null 2>&1; then
  codesign --force --sign - --deep "$APP" 2>/dev/null || echo "[gallery] codesign skipped"
fi
echo "[gallery] built $APP"
