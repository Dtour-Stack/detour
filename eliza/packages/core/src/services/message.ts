...
function buildActionRescuePrompt(
	...
	For image-generation requests, choose GENERATE_IMAGE with a prompt param instead of acknowledging that you can make the image.
...
function buildActionOnlyRescuePrompt(draftReply: string): string {
	...
	- If the user asks for an image, picture, drawing, poster, caricature, meme, banner, or visual artifact, choose GENERATE_IMAGE.
...
const IMAGE_GENERATION_REQUEST_PATTERNS = [
	/\b(?:generate|create|make|draw|paint|render|visuali[sz]e|illustrate|design)\b[\s\S]{0,120}\b(?:image|picture|pic|photo|art|illustration|caricature|poster|thumbnail|meme|banner|visual)\b/iu,
	/\b(?:image|picture|pic|photo|art|illustration|caricature|poster|thumbnail|meme|banner|visual)\b[\s\S]{0,120}\b(?:generate|create|make|draw|paint|render|visuali[sz]e|illustrate|design)\b/iu,
];
...
const ACTION_OWNERSHIP_TRIGGER_PATTERNS = [
	...
	...IMAGE_GENERATION_REQUEST_PATTERNS,
...
function findDirectOwnedActionSuggestion(
	...
	if (looksLikeExplicitImageGenerationRequest(messageText)) {
		const imageActionName = findRuntimeActionName(runtime, "GENERATE_IMAGE");
		if (imageActionName) {
			return {
				actionName: imageActionName,
				score: 100,
				secondBestScore: 0,
				reasons: ["direct:image-generation"],
			};
		}
	}
...
export function looksLikeExplicitImageGenerationRequest(text: string): boolean {
	const safe = text.length > 10_000 ? text.slice(0, 10_000) : text;
	return IMAGE_GENERATION_REQUEST_PATTERNS.some((pattern) =>
		pattern.test(safe),
	);
}

function findRuntimeActionName(
	runtime: Pick<IAgentRuntime, "actions">,
	canonicalName: string,
): string | null {
	const canonical = normalizeActionIdentifier(canonicalName);
	for (const action of runtime.actions ?? []) {
		if (normalizeActionIdentifier(action.name) === canonical) {
			return action.name;
		}
		if (
			(action.similes ?? []).some(
				(simile) => normalizeActionIdentifier(simile) === canonical,
			)
		) {
			return action.name;
		}
...
function buildImageGenerationPromptFromMessage(messageText: string): string {
	return messageText
		.replace(/\[Discord [^\]]+\]\s*/giu, "")
		.replace(/\b@[A-Za-z0-9_.-]+\s+\([^)]*\):\s*/u, "")
		.replace(/<@!?\d+>/gu, "")
		.replace(/\s+/gu, " ")
		.trim();
}

function responseHasImagePromptParam(
	responseContent: Content,
	actionName: string,
): boolean {
	const params = responseContent.params;
	if (!isRecord(params)) return false;
	if (typeof params.prompt === "string" && params.prompt.trim().length > 0) {
		return true;
	}
	const nested = params[actionName.trim().toUpperCase()] ?? params[actionName];
	return (
		isRecord(nested) &&
		typeof nested.prompt === "string" &&
		nested.prompt.trim().length > 0
	);
}

function responseSelectedAction(
	responseContent: Pick<Content, "actions">,
	actionName: string,
): boolean {
	const target = normalizeActionIdentifier(actionName);
	return (responseContent.actions ?? []).some(
		(action) => normalizeActionIdentifier(action) === target,
	);
}

function applyDirectImageGenerationPlan(
	runtime: Pick<IAgentRuntime, "actions">,
	...
	const actionName = findRuntimeActionName(runtime, "GENERATE_IMAGE");
	if (!actionName) {
		return false;
	}

	if (
		hasNonPassiveAction(responseContent) &&
		(!responseSelectedAction(responseContent, actionName) ||
			responseHasImagePromptParam(responseContent, actionName))
	) {
		return false;
	}

	const prompt = buildImageGenerationPromptFromMessage(messageText);
	if (!prompt) {
		return false;
	}

	const existingParams =
		responseContent.params && isRecord(responseContent.params)
			? responseContent.params
			: {};
	responseContent.actions = [actionName];
	responseContent.providers = [];
	responseContent.params = {
		...existingParams,
		[actionName.trim().toUpperCase()]: { prompt },
	};
	responseContent.text = "";
	responseContent.simple = false;
	return true;
}
...
export function withInferredContextRoutingFallback(
	...
	if (activeContexts.length > 0) {
		...
		const inferredMediaContexts = getActiveRoutingContexts(inferred).filter(
			(context) => context === "media" && !activeSet.has(context),
		);
		if (inferredMediaContexts.length === 0) {
			return routing;
		}
		return {
			...routing,
			secondaryContexts: [
				...
				...inferredMediaContexts,
			],
		};
	}
	if (getActiveRoutingContexts(inferred).length > 0) {
		return {
			...inferred,
...
Return TOON only with the continuation in the text field, starting immediately after the last character above.`;
		...
		if (applyDirectImageGenerationPlan(runtime, message, responseContent)) {
			runtime.logger.info(
				{
					src: "service:message",
					actions: responseContent.actions ?? [],
				},
				"Promoted explicit image request to image generation action",
			);
		}
