...
export class TaskDrain {
	...
	private async ensureTask(runtime: IAgentRuntime): Promise<void> {
		...
		const mine = existing.filter((t) => this.matchesTask(t, String(agentId)));
		const [primary, ...duplicates] = mine;
		if (primary?.id) {
			if (duplicates.length > 0 && typeof runtime.deleteTask === "function") {
				await Promise.all(
					duplicates
						.map((t) => t.id)
						.filter((id): id is UUID => id != null)
						.map((id) => runtime.deleteTask(id).catch(() => {})),
				);
			}
			this.taskId = primary.id;
