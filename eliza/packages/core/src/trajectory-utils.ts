import type { ActionAttempt, JsonValue } from "./features/trajectories/types";
import {
...
type TrajectoryLoggerLike = {
	...
	completeStep?: (
		...
		rewardInfo?: { reward?: number; components?: Record<string, JsonValue> },
	) => void;
...
function jsonValue(value: unknown, depth = 0): JsonValue {
	...
	if (Array.isArray(value)) {
		if (depth >= 4) return encodeToonValue({ value });
...
function actionSucceeded(result: unknown): boolean {
...
function actionResultSummary(result: unknown): Record<string, JsonValue> {
	const summary: Record<string, JsonValue> = { executed: true };
	if (isRecord(result)) {
		if (typeof result.text === "string") summary.text = result.text;
		if (typeof result.error === "string") summary.error = result.error;
		if (typeof result.data !== "undefined")
			summary.data = jsonValue(result.data);
		return summary;
	}
	if (typeof result !== "undefined") summary.value = jsonValue(result);
	return summary;
}

function completeActionTrajectoryStep(
	trajectoryLogger: TrajectoryLoggerLike,
	...
	const failed = typeof error !== "undefined";
	const success = failed ? false : actionSucceeded(result);
	const errorText = failed ? actionErrorText(error) : undefined;
	trajectoryLogger.completeStep(
		...
		stepId,
		{
			...
			reasoning: success
				...
				: `Action ${actionName} failed${errorText ? `: ${errorText}` : ""}`,
		},
		{ reward: success ? 0.1 : -0.1 },
	);
}
