...
export class AutonomyService extends Service {
	...
	private async initialize(): Promise<void> {
		...
		if (autonomyEnabled) {
			...
			await this.startAutonomyRunner();
			this.isRunning = true;
		} else {
			this.runtime.logger.info(
				{ src: "autonomy", agentId: this.runtime.agentId },
	...
	private autonomySchema(): Array<{
		...
		return [
			...
			{
				field: "providers",
				description: "List of providers to use for additional context",
				required: false,
			},
			{
				field: "actions",
			...
			{
				field: "simple",
				description: "Whether this is a simple response",
				required: false,
			},
		];
	}
	...
	private async runStructuredAutonomyCycle(): Promise<void> {
		const mode = this.getAutonomyMode();
		const context = await this.buildAutonomyContextForBatcher();
		const autonomousMessage: Memory = {
			id: stringToUuid(uuidv4()),
			entityId: this.autonomyEntityId,
			content: {
		...
		const fields = await this.runtime.dynamicPromptExecFromState({
			state: { values: {}, data: {}, text: context } as State,
			params: { prompt } as Omit<GenerateTextParams, "prompt"> & {
				prompt: string;
			},
			schema: this.autonomySchema(),
			options: {
				modelSize: "large",
				key: "autonomy:task-fallback",
			},
		});
	...
	private async startAutonomyRunner(): Promise<void> {
		if (this.runtime.promptBatcher) {
			this.registerAutonomyBatcherSection();
			return;
		}
		await this.ensureAutonomyTaskFallback();
	}

	private async ensureAutonomyTaskFallback(): Promise<void> {
		if (!this.runtime.getTaskWorker(AUTONOMY_TASK_NAME)) {
			this.runtime.registerTaskWorker({
				name: AUTONOMY_TASK_NAME,
				shouldRun: async (rt) => {
					const service = rt.getService(AUTONOMY_SERVICE_TYPE) as AutonomyService | null;
					return Boolean(rt.enableAutonomy && service?.isLoopRunning());
				},
				execute: async (rt) => {
					const service = rt.getService(AUTONOMY_SERVICE_TYPE) as AutonomyService | null;
					await service?.triggerStructuredThinkNow();
					return undefined;
				},
			});
		}

		const tags = ["queue", ...AUTONOMY_TASK_TAGS];
		const existing = await this.runtime.getTasks({
			tags,
			agentIds: [this.runtime.agentId],
		});
		const matches = existing.filter((task) => task.name === AUTONOMY_TASK_NAME);
		const [primary, ...duplicates] = matches;
		for (const duplicate of duplicates) {
			if (duplicate.id) await this.runtime.deleteTask(duplicate.id).catch(() => {});
		}
		const metadata = {
			...(primary?.metadata ?? {}),
		...
		if (primary?.id) {
			await this.runtime.updateTask(primary.id, {
	...
	private async removeAutonomyTaskFallback(): Promise<void> {
		const existing = await this.runtime.getTasks({
			tags: ["queue", ...AUTONOMY_TASK_TAGS],
			agentIds: [this.runtime.agentId],
		});
		for (const task of existing) {
			if (task.id && task.name === AUTONOMY_TASK_NAME) {
				await this.runtime.deleteTask(task.id).catch(() => {});
			}
		}
	}

	private async syncAutonomyTaskFallbackInterval(): Promise<void> {
		const existing = await this.runtime.getTasks({
			tags: ["queue", ...AUTONOMY_TASK_TAGS],
			agentIds: [this.runtime.agentId],
		});
		for (const task of existing) {
			if (!task.id || task.name !== AUTONOMY_TASK_NAME) continue;
	...
	async disableAutonomy(): Promise<void> {
		...
		if (this.isRunning) {
			this.runtime.promptBatcher?.removeSection("autonomy");
			if (!this.runtime.promptBatcher) {
				await this.removeAutonomyTaskFallback();
			}
	...
	async triggerStructuredThinkNow(): Promise<boolean> {
		if (this.isThinking) {
			this.runtime.logger.info(
				{ src: "autonomy", agentId: this.runtime.agentId },
		...
		this.runtime.logger.info(
			{ src: "autonomy", agentId: this.runtime.agentId },
			"Manually triggered structured autonomous thinking",
		);

		this.isThinking = true;
		try {
			await this.runStructuredAutonomyCycle();
			return true;
		} catch (error) {
			this.runtime.logger.error(
				{ src: "autonomy", agentId: this.runtime.agentId, error },
				"Error during structured autonomous think",
			);
			return false;
		} finally {
