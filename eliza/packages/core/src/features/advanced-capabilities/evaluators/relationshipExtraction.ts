...
function isLikelyPersonName(name: string): boolean {
	...
	return parts.every((part) => {
		if (NON_PERSON_WORDS.has(part.toLowerCase())) return false;
...
function extractMentionedPeople(text: string): MentionedPerson[] {
	...
	for (const pattern of patterns) {
		...
		while (patternMatch !== null) {
			const name = patternMatch[1] ? normalizeMentionName(patternMatch[1]) : "";
			const key = name.toLowerCase();
			if (isLikelyPersonName(name) && !seen.has(key)) {
				seen.add(key);
				people.push({
					name,
...
async function createOrUpdateMentionedEntity(
	...
	const name = normalizeMentionName(person.name);
	if (!isLikelyPersonName(name)) return;

	const existing = (
		await runtime.getEntitiesByNames({ names: [name], agentId: runtime.agentId })
	).find((entity) =>
		entity.names.some((entityName) => entityName.toLowerCase() === name.toLowerCase()),
	) ?? null;

	if (!existing) {
		await runtime.upsertEntities([{
			id: stringToUuid(`mentioned:${runtime.agentId}:${name.toLowerCase()}`),
			agentId: runtime.agentId,
			names: [name],
			metadata: {
