...
function readRuntimeIntegerSetting(
...
function readRuntimeListSetting(runtime: IAgentRuntime, key: string): string[] {
	const raw = runtime.getSetting(key);
...
function roleMentionsAnyName(
	...
	for (const role of roles.values()) {
		if (textMentionsAnyName(role.name, names)) {
			return true;
		}
	}
	return false;
...
function contentMentionsConfiguredRole(
	content: string | undefined,
	roleIds: string[],
): boolean {
	if (!content || roleIds.length === 0) {
		return false;
	}
	const mentionedRoleIds = new Set(
		Array.from(content.matchAll(/<@&(\d+)>/g), (match) => match[1]),
	);
	return roleIds.some((id) => mentionedRoleIds.has(id));
}
...
const ATTACHMENT_TEXT_INLINE_LIMIT = 24000;
const ATTACHMENT_TEXT_TOTAL_LIMIT = 48000;

function inlineAttachmentSections(attachments: Media[]): string {
	const sections: string[] = [];
	let remaining = ATTACHMENT_TEXT_TOTAL_LIMIT;

	for (const media of attachments) {
		const text = typeof media.text === "string" ? media.text.trim() : "";
		if (!text || remaining <= 0) continue;

		const title =
			typeof media.title === "string" && media.title.trim().length > 0
				? media.title.trim()
				: media.id;
		const sliceLength = Math.min(
			text.length,
			ATTACHMENT_TEXT_INLINE_LIMIT,
			remaining,
		);
		const clipped = text.slice(0, sliceLength);
		const suffix = text.length > sliceLength ? "\n[truncated]" : "";
		const section = `[Attachment: ${title}]\n${clipped}${suffix}`;
...
function appendReadableAttachmentText(
	content: string,
	attachments: Media[],
): string {
	const sections = inlineAttachmentSections(attachments);
	if (!sections) return content;
	const base = content.trim().length > 0 ? content.trimEnd() : "";
	return base ? `${base}\n\n${sections}` : sections;
}
...
export class MessageManager {
	...
	constructor(discordService: IDiscordService, runtime: ICompatRuntime) {
		...
		this.statusReactionScope = (
			["all", "group-mentions", "none"].includes(reactionScopeSetting ?? "")
				...
				// Default "all" so DMs + guild channels both get the ⏳/🤔/✅
				// reactions. Previously defaulted to "group-mentions" which
				// silently dropped feedback in DMs and addressed-by-name turns.
				: "all"
	...
	private async emitObservedMessage(memory: Memory): Promise<void> {
		try {
			...
			this.runtime.logger.warn(
				...
				"Failed to emit observed Discord message",
			);
		}
	}
    ...
    */
		...
		try {
			...
			const roleMentionsBot =
				roleMentionsAnyName(message, botAddressNames) ||
				contentMentionsConfiguredRole(
					processedContent,
					readRuntimeListSetting(this.runtime, "DISCORD_MENTION_ROLE_IDS"),
				);
			...
			const outboundReplyToMessageId =
				!isDM && replyToMode !== "off" && isAddressedToBot
					? message.id
					: undefined;
			const strictModeShouldProcess = isDM || isAddressedToBot;
			...
			// Early acknowledgement: as soon as we know this message is
			// addressed to us, drop a ⏳ reaction so the user sees we're
			// on it, even before model rebuild / memory build / planner
			// kick in. This is best-effort — if the bot lacks "Add
			// Reactions" permission the call silently fails (eats the
			// promise rejection). Late-stage transitions to 🤔 (thinking)
			// and ✅/❌ (terminal) happen below in the existing pipeline.
			const willRespond =
				strictModeShouldProcess &&
				!ignoresOtherTarget &&
				this.discordSettings.autoReply;
			const earlyAck = willRespond
			...
			const newMessage = await this.discordService.buildMemoryFromMessage(
				message,
				{
					...
					extraContent: {
						mentionContext: {
							isMention: isBotMentioned || roleMentionsBot,
							isReply: isReplyToBot,
							isThread: isInThread,
							mentionType:
								isBotMentioned || roleMentionsBot
									? "platform_mention"
									: isReplyToBot
										? "reply"
										: isInThread
			...
			this.runtime.logger.info(
				{
					...
					channelId: message.channel.id,
					channelType: type,
					autoReply: this.discordSettings.autoReply,
					strictMode: strictModeEnabled,
					isMention: isBotMentioned,
					isNameMention: explicitlyAddressesBotByName,
					isRoleMention: roleMentionsBot,
					isReply: isReplyToBot,
					isDM,
				},
				this.discordSettings.autoReply
					? "Discord message passed auto-reply gate"
					: "Discord message ingested without auto-reply",
			);
			...
			if (
				...
				await this.emitObservedMessage(newMessage);
				this.runtime.logger.debug(
					{
						src: "plugin:discord",
						agentId: this.runtime.agentId,
			...
			this.runtime.logger.info(
				...
				"Routing Discord message to runtime",
			);
			...
			const useReactions = shouldShowStatusReaction(
				...
				message,
				clientUserId,
				explicitlyAddressesBotByName || roleMentionsBot,
			);
			...
			let reactionStatusAckSent = false;
			...
			const sendStatusUpdate = async (text: string) => {
				await channel.send({
					content: text,
					...(outboundReplyToMessageId && replyToMode !== "off"
						? {
				...
				? createStatusReactionController(message, {
						onError: (reactionError, emoji) => {
							this.runtime.logger.warn(
								...
								"Discord status reaction failed",
							);
							if (responseEmitted || reactionStatusAckSent) {
								return;
							}
							reactionStatusAckSent = true;
							void sendStatusUpdate("seen, working.").catch((statusError) => {
								this.runtime.logger.warn(
									{
										src: "plugin:discord",
										agentId: this.runtime.agentId,
									...
									"Failed to send Discord reaction status update",
								);
							});
						},
					})
				: null;
			const longRunningStatus = createLongRunningStatusController({
				firstDelayMs: readRuntimeIntegerSetting(
					this.runtime,
					"DISCORD_STATUS_FIRST_MS",
					8_000,
					1_000,
				),
				intervalMs: readRuntimeIntegerSetting(
					this.runtime,
					"DISCORD_STATUS_INTERVAL_MS",
					45_000,
					5_000,
				),
				maxUpdates: readRuntimeIntegerSetting(
					this.runtime,
					"DISCORD_STATUS_MAX_UPDATES",
					3,
					0,
				),
				...
				onError: (statusError) => {
					this.runtime.logger.warn(
						...
						"Failed to send Discord status update",
					);
				},
			});
	...
	async processMessage(
		...
		processedContent = appendReadableAttachmentText(
			processedContent,
			attachments,
		);
