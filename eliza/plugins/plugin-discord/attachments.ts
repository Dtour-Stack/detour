...
function plaintextDescription(text: string): string {
...
export class AttachmentManager {
	...
	private async processPlaintextAttachment(
		...
		try {
			...
			let summary: { title: string; description: string } | null = null;
			try {
				summary = await generateSummary(this.runtime, text);
			} catch (error) {
				this.runtime.logger.warn(
					{
						src: "plugin:discord",
					...
					"Plaintext attachment summary failed",
				);
			}
			...
			return {
				id: attachment.id,
				url: attachment.url,
				title: summary?.title || attachment.name || "Plaintext Attachment",
				source: "Plaintext",
				contentType: ContentType.DOCUMENT,
				description: summary?.description || plaintextDescription(text),
				text,
			};
		} catch (error) {
			this.runtime.logger.error(
