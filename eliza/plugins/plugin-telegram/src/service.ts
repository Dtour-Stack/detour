...
export class TelegramService extends Service {
  ...
  private async initializeBot(): Promise<void> {
    ...
    // FIX: Telegraf 4.x's `bot.launch()` returns a promise that does NOT
    // resolve until `bot.stop()` is called — it's the bot's lifecycle
    // promise, not a "ready" signal. Awaiting it hangs forever. Fire-and-
    ...
    const launchOnce = (attempt: number): void => {
      bot
        ...
        .catch((err) => {
          ...
          setTimeout(() => launchOnce(attempt + 1), delayMs);
        });
    };
    launchOnce(1);
    if (botToken) {
    ...
    // Verify the polling loop is actually running by fetching bot info.
    // Will throw if the token is invalid or Telegram is unreachable, which
    // surfaces a real failure instead of letting Telegraf silently retry.
  ...
  private setupOwnerPairCommand(): void {
    this.bot?.command('eliza_pair', async (ctx) => {
      try {
        ...
        logger.error(
          ...
          'Error handling /eliza_pair command',
        );
      }
    });
  }
  ...
  private async handleNewChat(ctx: Context): Promise<void> {
    ...
    const world: World = {
      ...
      name: chatTitle,
      agentId: this.runtime.agentId,
      // FIX: Eliza's SQL plugin requires `messageServerId` to be a UUID.
      // Passing the raw Telegram chat id (a numeric like -100123...) makes
      // the SQL adapter warn "Ignoring non-UUID message/server identifier for
      // world" and silently drop the world row, which kills inbound message
      // routing. Derive a deterministic UUID from the chat id instead.
      messageServerId: createUniqueUuid(this.runtime, String(chatId)) as UUID,
      metadata: {
        source: 'telegram',
    ...
    const generalRoom: Room = {
      ...
      type: channelType,
      channelId: chatId,
      // FIX: serverId must be a UUID per eliza's SQL adapter — see
      // messageServerId comment above.
      serverId: createUniqueUuid(this.runtime, String(chatId)) as UUID,
      worldId,
    };
  ...
  private async buildForumTopicRoom(
    ...
    try {
      ...
      const room: Room = {
        ...
        // FIX: serverId must be a UUID — see comment in main room block.
        serverId: createUniqueUuid(this.runtime, String(chatId)) as UUID,
        worldId,
        metadata: {
          threadId,
