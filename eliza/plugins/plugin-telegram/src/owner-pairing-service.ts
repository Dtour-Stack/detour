  ...
  implements TelegramOwnerPairingService
{
  ...
  static async start(runtime: IAgentRuntime): Promise<Service> {
    ...
    if (resolveVerifyService(runtime)) {
      const registered = service.registerPairCommand(runtime);
      if (!registered) service.deferPairCommandRegistration(runtime);
      logger.info(
        {
          src: 'plugin:telegram:owner-pairing',
        ...
        registered
          ...
          : 'TelegramOwnerPairingService started; /eliza_pair will register when TelegramService is ready',
      );
    } else {
      logger.info(
        {
   ...
   */
  private registerPairCommand(runtime: IAgentRuntime): boolean {
    ...
    if (!telegramSvc || typeof telegramSvc !== 'object') {
      logger.debug(
        ...
        'TelegramService unavailable during owner-pairing start',
      );
      return false;
    }
    ...
    if (
      ...
      logger.debug(
        { src: 'plugin:telegram:owner-pairing', agentId: runtime.agentId },
        'Telegraf bot instance not available during owner-pairing start',
      );
      return false;
    }
  ...
  private deferPairCommandRegistration(runtime: IAgentRuntime): void {
    const loadPromise = (runtime as unknown as {
      getServiceLoadPromise?: (type: string) => Promise<unknown>;
    }).getServiceLoadPromise?.(TELEGRAM_SERVICE_NAME);
    if (!loadPromise) return;
    void loadPromise
      .then(() => {
        const registered = this.registerPairCommand(runtime);
        if (registered) {
          logger.info(
            { src: 'plugin:telegram:owner-pairing', agentId: runtime.agentId },
            '/eliza_pair command registered after TelegramService became ready',
          );
        }
      })
      .catch((err) => {
        logger.warn(
          ...
          'TelegramService failed before /eliza_pair command could register',
        );
      });
  }
