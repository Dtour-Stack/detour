...
export const reviewPrAction: Action = {
  ...
  handler: async (
    ...
    // Self-action guard: refuse to review/comment on a PR the same
    // identity opened. Prevents the agent from talking to itself when
    // it both authors and reviews PRs.
    try {
      const [selfLogin, prResp] = await Promise.all([
        getAuthenticatedLogin(runtime, identity),
        resolved.client.pulls.get({ owner: parts.owner, repo: parts.name, pull_number: number }),
      ]);
      const prAuthor = (prResp.data.user?.login ?? null) as string | null;
      if (isSameLogin(prAuthor, selfLogin)) {
        const err = `Refusing to ${action} ${repo}#${number}: PR was authored by the same identity (@${selfLogin}). Self-action guard.`;
        await callback?.({ text: err });
        return { success: false, error: err };
      }
    } catch (err) {
      // Author lookup failed — log and proceed. Don't hard-fail the
      // legitimate action on a transient GitHub error.
      logger.warn(
        { error: errorMessage(err) },
        "[GitHub:REVIEW_PR] self-author check failed, continuing",
      );
    }
